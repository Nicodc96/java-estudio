package talentotech.controladores;
...
import talentotech.entidades.Producto;
import talentotech.menu.Menu;
import talentotech.validaciones.Validador;
...
public class ControladorProducto {
    ...
    public static void manejarAgregarProducto(Scanner scanner, List<Producto> listaProductos) {
        try {
            ...
            System.out.printf("""
                    ...
                    Ingrese "si" para confirmar o "no" para cancelar: """, nombreProducto, precioProducto,
                    descripcionProducto);
            datoIngresado = scanner.nextLine();
    ...
    public static void manejarModificarProducto(List<Producto> listaDeProductos, Scanner scanner) {
        ...
        for (Producto producto : listaDeProductos) {
            sBInfoProductos.append(String.format("- Nombre: %s | ID: %d", producto.getNombre(), producto.getId()))
            ...
            switch (Menu.menuModificarProducto(scanner, sBInfoProductos.toString())) {
                case 1:
                    try {
                        ...
                        if (Objects.isNull(productoAMostrar))
                            break;
                        System.out.println("\n" + productoAMostrar);
                    } catch (InputMismatchException e) {
                ...
                case 2:
                    try {
                        ...
                        System.out.print(
                                "Ingrese el nuevo precio (ingrese -1 para cancelar o -2 para mantener el precio): ");
                        datoIngresado = scanner.nextLine();
                        if (datoIngresado.equals("-1")) {
                            System.out.println("Operación cancelada.");
                            break;
                        ...
                        nuevoPrecio = datoIngresado.equals("-2") ? productoAModificar.getPrecio()
                                : Double.parseDouble(datoIngresado);

                        System.out.println(
                                "Ingrese la nueva descripción (ingrese -1 para cancelar o -2 para mantener la descripción): ");
                        if (datoIngresado.equals("-1")) {
                            System.out.println("Operación cancelada.");
                            break;
                        }
