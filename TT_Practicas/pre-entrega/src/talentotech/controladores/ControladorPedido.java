            ...
            switch (Menu.menuCreacionPedido(scanner, pedidoAGenerar.getId())) {
                case 1:
                    try {
                        ...
                        pedidoAGenerar.agregarProducto(
                                listaProductos.stream().filter(p -> p.getId() == idIngresado).toList().get(0));
                        System.out.println("Se ha agregado el producto al pedido correctamente.");
                        break;
                    } catch (InputMismatchException e) {
                        System.out.print("""
                                        **ERROR**
                                ¡Se debe ingresar un número como ID!
                                """);
                    }
                    break;
                case 2:
                    try {
                        ...
                        pedidoAGenerar.getListaProductos().remove(pedidoAGenerar.getListaProductos().stream()
                                .filter(p -> p.getId() == idIngresado).toList().get(0));
